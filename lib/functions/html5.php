...
add_action( 'init', 's_register_html5_scripts' );
/**
 ...
 */
function s_register_html5_scripts() {
	 ...
	 */
	//wp_register_script( 'gs-html5-shiv', CHILD_JS . '/' . s_script_suffix( 'html5shiv', 'js' ), array(), '3.6.2pre' );
	 ...
	 */
	wp_register_script( 'gs-modernizr', CHILD_JS . '/' . s_script_suffix( 'modernizr', 'js' ), array(), '2.6.3pre' );
	//wp_register_script( 'gs-modernizr', 'http://cdnjs.cloudflare.com/ajax/libs/modernizr/2.6.2/modernizr.min.js', array(), '2.6.2', false );
	
}

add_action( 'wp_head', 's_load_html5shiv', 99 );
/** 
 * Add HTML5Shiv
 ...
 */
function s_load_html5shiv() {
	global $is_IE;
	
	if ( $is_IE ) {
	?>
<!--[if lt IE 9]>
<script src="<?php echo CHILD_JS . '/' . s_script_suffix( 'html5shiv', 'js' ); ?>"></script>
<![endif]-->
	<?php
	}
}

add_action( 'wp_enqueue_scripts', 's_load_html5_scripts', 99 );
/** 
 * Add Modernizer
 */
function s_load_html5_scripts() {
